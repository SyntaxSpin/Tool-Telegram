...
class AppPreferencesRepository(private val dataStore: DataStore<Preferences>) {
  val isUseMonet = dataStore.data.map { it[isUseMonetPreference] ?: true }
  val isUseHighContrast = dataStore.data.map { it[isUseHighContrastPreference] ?: false }

  suspend fun enableMonet(value: Boolean) {
    dataStore.edit { preferences -> preferences[isUseMonetPreference] = value }
  }

  suspend fun enableHighContrast(value: Boolean) {
    dataStore.edit { preferences -> preferences[isUseHighContrastPreference] = value }
  }
}
